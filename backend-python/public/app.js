...
async function connect() {
    ...
    try {
        ...
        ws.onopen = () => {
            ...
            // #region agent log
            fetch('http://127.0.0.1:7245/ingest/15e3bf2d-33c4-432b-b85e-7b4628f43030',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'app.js:109',message:'WebSocket opened, sending connect',data:{sessionId,wsReadyState:ws.readyState},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'O'})}).catch(()=>{});
            // #endregion
...
function setupAudioProcessing() {
    ...
    processor.onaudioprocess = (e) => {
        // #region agent log
        if (!ws || ws.readyState !== WebSocket.OPEN || !sessionId || !isConnected) {
            fetch('http://127.0.0.1:7245/ingest/15e3bf2d-33c4-432b-b85e-7b4628f43030',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'app.js:154',message:'Audio blocked from sending',data:{hasWs:!!ws,wsReadyState:ws?.readyState,hasSessionId:!!sessionId,isConnected,wsOpen:ws?.readyState===WebSocket.OPEN},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'O'})}).catch(()=>{});
            return;
        }
        // #endregion
        ...
        // #region agent log
        fetch('http://127.0.0.1:7245/ingest/15e3bf2d-33c4-432b-b85e-7b4628f43030',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'app.js:163',message:'Sending audio chunk',data:{dataLen:inputData.length,pcmDataLen:pcmBlob.data?.length||0,hasData:!!pcmBlob.data},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'O'})}).catch(()=>{});
        // #endregion
    ...
    switch (message.type) {
        case 'status':
            ...
            if (status === 'CONNECTED') {
                ...
                isConnected = true;
                // #region agent log
                fetch('http://127.0.0.1:7245/ingest/15e3bf2d-33c4-432b-b85e-7b4628f43030',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'app.js:174',message:'CONNECTED status received, isConnected set to true',data:{sessionId,wsReadyState:ws?.readyState},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'O'})}).catch(()=>{});
                // #endregion
                connectBtn.disabled = true;
...
async function playAudio(base64Audio) {
    ...
    try {
        ...
        // #region agent log
        if (len > 0) {
            fetch('http://127.0.0.1:7245/ingest/15e3bf2d-33c4-432b-b85e-7b4628f43030',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'app.js:227',message:'Playing audio chunk',data:{base64Len:base64Audio.length,binaryLen:len,bytesLen:bytes.length,firstBytes:[bytes[0],bytes[1],bytes[2],bytes[3]]},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'N'})}).catch(()=>{});
        }
        // #endregion

        // Convert PCM bytes to Int16Array (16-bit little-endian)
        // Each sample is 2 bytes, so we need to create Int16Array with proper byte offset
        ...
        // Create Int16Array view of the bytes buffer
        // After slice(), bytes.buffer might have a different offset, so use bytes.byteOffset
        ...
        for (let i = 0; i < frameCount; i++) {
            // Int16 range: -32768 to 32767, normalize to -1.0 to 1.0
            channelData[i] = Math.max(-1, Math.min(1, dataInt16[i] / 32768.0));
        }
